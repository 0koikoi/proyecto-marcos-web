-- ============================================================
--  HUELLITAS — Esquema de Base de Datos v3.0
--  Clínica Veterinaria
--  Fecha: 2026-07-10
-- ============================================================
--  CAMBIOS RESPECTO A v2.0:
--    1. Tabla personal: agrega columna 'email'
--    2. Tabla solicitud_material: agrega columna 'cantidad_entregada'
--    3. Tabla vacuna: agrega FK opcional 'historia_clinica_id'
--       para trazabilidad clínica completa
--    4. Tabla personal: actualiza semilla con emails de ejemplo
--
--  INSTRUCCIONES DE MIGRACIÓN (si ya tienes la v2 instalada):
--    Ejecutar SOLO la sección "MIGRACIÓN v2 → v3" al final.
--    Si instalas desde cero, ejecutar el script completo.
--
--  Contraseña inicial de todos los usuarios: Huellitas2025!
-- ============================================================
...
-- ============================================================
-- v3: Agrega columna 'email' para notificaciones internas
CREATE TABLE personal (
    ...
    telefono              VARCHAR(15),
    email                 VARCHAR(150),                    -- v3: correo institucional
...
INSERT INTO personal
    (codigo_institucional, nombre_completo, rol_id, cargo, especialidad, telefono, email, username, password_hash)
VALUES
    -- ADMINISTRADORES (rol_id = 1)
    ('C000001', 'Administrador Principal',    1, 'Administrador', NULL,                NULL, 'admin1@huellitas.com', 'admin1', crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000002', 'Administrador Auxiliar',     1, 'Administrador', NULL,                NULL, 'admin2@huellitas.com', 'admin2', crypt('Huellitas2025!', gen_salt('bf', 12))),

    -- RECEPCIÓN (rol_id = 2)
    ('C000003', 'Recepcionista Uno',          2, 'Recepcionista', NULL,                NULL, 'recep1@huellitas.com', 'recep1', crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000004', 'Recepcionista Dos',          2, 'Recepcionista', NULL,                NULL, 'recep2@huellitas.com', 'recep2', crypt('Huellitas2025!', gen_salt('bf', 12))),

    -- VETERINARIOS (rol_id = 3)
    ('C000005', 'Dr. Veterinario Uno',        3, 'Veterinario', 'Medicina General',    NULL, 'vet1@huellitas.com',   'vet1',   crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000006', 'Dr. Veterinario Dos',        3, 'Veterinario', 'Medicina General',    NULL, 'vet2@huellitas.com',   'vet2',   crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000007', 'Dr. Veterinario Tres',       3, 'Veterinario', 'Cirugía',             NULL, 'vet3@huellitas.com',   'vet3',   crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000008', 'Dr. Veterinario Cuatro',     3, 'Veterinario', 'Dermatología',        NULL, 'vet4@huellitas.com',   'vet4',   crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000009', 'Dr. Veterinario Cinco',      3, 'Veterinario', 'Odontología',         NULL, 'vet5@huellitas.com',   'vet5',   crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000010', 'Dr. Veterinario Seis',       3, 'Veterinario', 'Nutrición Animal',    NULL, 'vet6@huellitas.com',   'vet6',   crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000011', 'Dr. Veterinario Siete',      3, 'Veterinario', 'Medicina General',    NULL, 'vet7@huellitas.com',   'vet7',   crypt('Huellitas2025!', gen_salt('bf', 12))),
    ('C000012', 'Dr. Veterinario Ocho',       3, 'Veterinario', 'Medicina en Exóticos',NULL, 'vet8@huellitas.com',   'vet8',   crypt('Huellitas2025!', gen_salt('bf', 12)));
...
CREATE TABLE historia_clinica (
    ...
    cita_id         BIGINT        REFERENCES cita(id),               -- NULL = walk-in
...
-- 6. VACUNAS
-- ============================================================
-- v3: Agrega FK opcional historia_clinica_id para trazabilidad completa
CREATE TABLE vacuna (
    id                  BIGSERIAL    PRIMARY KEY,
    paciente_id         BIGINT       NOT NULL REFERENCES paciente(id),
    personal_id         BIGINT       NOT NULL REFERENCES personal(id),
    historia_clinica_id BIGINT       REFERENCES historia_clinica(id), -- v3: trazabilidad clínica
...
-- ── 8.3 SOLICITUDES DE MATERIAL ────────────────────────────
-- v3: Agrega columna 'cantidad_entregada' para aprobaciones parciales
CREATE TABLE solicitud_material (
    id                    BIGSERIAL    PRIMARY KEY,
    solicitante_id        BIGINT       NOT NULL REFERENCES personal(id),
    producto_id           BIGINT       NOT NULL REFERENCES producto(id),
    cantidad_solicitada   INT          NOT NULL CHECK (cantidad_solicitada > 0),
    cantidad_entregada    INT          CHECK (cantidad_entregada > 0),  -- v3: puede ser < solicitada
    motivo                TEXT         NOT NULL,
    estado                VARCHAR(20)  NOT NULL DEFAULT 'PENDIENTE'
...
CREATE TABLE venta (
    id              BIGSERIAL     PRIMARY KEY,
    nro_boleta      VARCHAR(20)   NOT NULL UNIQUE,
...
CREATE INDEX idx_vacuna_historia        ON vacuna(historia_clinica_id);  -- v3
...
-- ============================================================
-- MIGRACIÓN v2 → v3 (solo si ya tienes la v2 instalada)
-- ============================================================
-- Si ya tienes el esquema v2 y SOLO quieres aplicar los cambios,
-- ejecutar únicamente estas líneas (no el script completo):
--
--   ALTER TABLE personal
--       ADD COLUMN IF NOT EXISTS email VARCHAR(150);
--
--   ALTER TABLE solicitud_material
--       ADD COLUMN IF NOT EXISTS cantidad_entregada INT CHECK (cantidad_entregada > 0);
--
--   ALTER TABLE vacuna
--       ADD COLUMN IF NOT EXISTS historia_clinica_id BIGINT REFERENCES historia_clinica(id);
--
--   CREATE INDEX IF NOT EXISTS idx_vacuna_historia ON vacuna(historia_clinica_id);
--
-- ============================================================
